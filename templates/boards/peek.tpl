...
{$is_writeable = Context_ProjectBoard::isWriteableByActor($dict, $active_worker)}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=project_board&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=project_board&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});
