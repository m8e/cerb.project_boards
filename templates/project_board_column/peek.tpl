...
{$is_writeable = Context_ProjectBoardColumn::isWriteableByActor($dict, $active_worker)}
	...
	$popup.one('popup_open',function(event,ui) {
		...
		$popup.find('button.cerb-peek-edit')
			...
			.on('cerb-peek-saved', function(e) {
				...
				var evt = new jQuery.Event('cerb-project-column-changed');
				evt.board_id = {$dict->board_id|default:0};
				evt.column_id = {$dict->id|default:0};
				$(document).trigger(evt);
			})
			.on('cerb-peek-deleted', function(e) {
				genericAjaxPopupClose($layer);
				
				var evt = new jQuery.Event('cerb-project-column-deleted');
				evt.board_id = {$dict->board_id|default:0};
				evt.column_id = {$dict->id|default:0};
				$(document).trigger(evt);
			})
		...
		// Links
		$popup.find('div.cerb-links-container')
			.on('cerb-links-changed', function(e) {
				var evt = new jQuery.Event('cerb-project-column-changed');
				evt.board_id = {$dict->board_id|default:0};
				evt.column_id = {$dict->id|default:0};
				$(document).trigger(evt);
		...
		$popup.find('.cerb-peek-profile').click(function(e) {
			if(e.shiftKey || e.metaKey) {
				window.open('{devblocks_url}c=profiles&type=project_board_column&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank');
				
			} else {
				document.location='{devblocks_url}c=profiles&type=project_board_column&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
			}
		});
